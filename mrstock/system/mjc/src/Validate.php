...
class Validate
{
    ...
    private static $defaultValidator = array(
     ...
     * @var array
     */
    private static $validator = array();
        ...
        foreach ($this->validateparam as $k => $v) {
            ...
            if ($this->validateparam[$k]['result'] && $v['input'] != "") {
                switch ($v['validator']) {
                    ...
                    case "length":
                        // 判断编码取字符串长度
                        $input_encode = mb_detect_encoding($v['input'], array(
                            'UTF-8',
                            'GBK',
                            'ASCII'
                        ));
                    ...
                    default:
                        ...
                        $this->validateparam[$k]['result'] = $this->check($v['input'], self::$validator[$v['validator']]);
                }
            }
        }
        $error = $this->getError();
    ...
    private function GetConfig()
    {
        if (empty(self::$validator)) {
            ...
            $validateConfig = Config::get("validate");
            if ($validateConfig['validator']) {
                self::$validator = array_merge(self::$validator, $validateConfig['validator']);
            }
        }
    }
     ...
     */
    public function check($str = '', $validator = '')
    {
        if (isset(self::$validator[$validator])) {
     ...
     * 需要验证的内容
     *
     * @param array $validateparam
     *            array("input"=>"","require"=>"","validator"=>"","regexp"=>"","operator"=>"","to"=>"","min"=>"","max"=>"",message=>"")
     *            input要验证的值
    ...
    public function setValidate($validateparam)
    {
        ...
        $this->validateparam = array_merge($this->validateparam, array(
            $validateparam
        ));
    }
