...
class RpcClient
{
    ...
    protected function setConnection()
    {
        ...
        if (!$this->openConnection()) {
            throw new \Exception("can not connect to server".$this->curAddress, -__LINE__);
        }
    }
    ...
    protected function openConnection()
    {
        ...
        //self::$addressArray[1]="192.168.10.73:5611";
        //print_r(self::$addressArray);exit;
        $addressArray = $this->parseAddressArray(self::$addressArray, []);
        foreach ($addressArray as $address) {
            //排除正在重启的服务地址
            // if(!empty($this->badAddress)&&in_array($address, $this->badAddress)){
            //     continue;
            // }
