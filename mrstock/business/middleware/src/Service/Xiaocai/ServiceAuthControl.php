...
namespace MrStock\Business\Middleware\Service\Xiaocai;
    ...
    private function checkAuthenticate()
    {
        ...
        if (in_array(strtolower($v), ["cloud", "institution", "company"])) {
            ...
            if (empty($member_id) || empty($key)) {
                throw new \Exception('用户账号登录参数错误', '-1001');
            }
            $res = \MrstockCloud\Client\MrstockCloud::auth()->inneruse()->v()->auth()->member(["member_id" => $member_id, "key" => $key])->request();
            if ($res["code"] != 1) {
                throw new \Exception($res["message"], $res["code"]);
            }
        }
